...
export class WebhookMpController {
  ...
  @ApiOperation({
    ...
    description:
      'Persiste en `Payments`. Si `status=processed` y `payment_status_detail=accredited`, suma `total_amount` al `Balance`. Opcional `id_history_balance`: marca ese registro `isPaid=1` (sin crear otro historial).',
  })
